...
gboolean get_key_string(GKeyFile* key_file,
                        const gchar* group,
                        const gchar* key,
                        gchar** value,
...
gboolean get_key_bool(GKeyFile* key_file,
                      const gchar* group,
                      const gchar* key,
                      gboolean* value,
    ...
    gboolean val_false = (g_strcmp0(val, "0") == 0 || g_ascii_strcasecmp(val, "no") == 0 ||
                          g_ascii_strcasecmp(val, "false") == 0);
    if (val_false) {
        *value = FALSE;
        return TRUE;
    }
    gboolean val_true = (g_strcmp0(val, "1") == 0 || g_ascii_strcasecmp(val, "yes") == 0 ||
...
gboolean get_key_int(GKeyFile* key_file,
                     const gchar* group,
                     const gchar* key,
                     gint* value,
...
gboolean get_group(GKeyFile* key_file, const gchar* group, GHashTable** hash, GError* error) {
    ...
    *hash = g_hash_table_new(g_str_hash, g_str_equal);
    ...
    for (key = 0; key < num_keys; key++) {
        value = g_key_file_get_value(key_file, group, keys[key], &error);
...
struct config* load_config_file(const gchar* config_file, GError** error) {
    ...
    if (!get_key_bool(ini_file, "client", "ssl", &config->ssl, DEFAULT_SSL, error))
        return NULL;
    if (!get_key_bool(ini_file, "client", "ssl_verify", &config->ssl_verify, DEFAULT_SSL_VERIFY,
                      error))
        return NULL;
    if (!get_group(ini_file, "device", &config->device, *error))
        return NULL;

    if (!get_key_int(ini_file, "client", "connect_timeout", &val_int, DEFAULT_CONNECTTIMEOUT,
                     error))
        return NULL;
