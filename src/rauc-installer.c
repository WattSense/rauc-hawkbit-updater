 ...
 */
static void on_installer_status(GDBusProxy* proxy,
                                GVariant* changed,
    ...
    if (context->notify_event) {
        ...
        if (g_variant_lookup(changed, "Operation", "s", &message)) {
            ...
            g_queue_push_tail(&context->status_messages,
                              g_strdup_printf("%3" G_GINT32_FORMAT "%% %s", percentage, message));
...
gpointer install_loop_thread(gpointer data) {
    ...
    r_installer_proxy =
        r_installer_proxy_new_for_bus_sync(bus_type, G_DBUS_PROXY_FLAGS_GET_INVALIDATED_PROPERTIES,
    ...
    if (g_signal_connect(r_installer_proxy, "g-properties-changed", G_CALLBACK(on_installer_status),
                         context) <= 0) {
        g_printerr("Failed to connect properties-changed signal\n");
        goto out_loop;
    }
    if (g_signal_connect(r_installer_proxy, "completed", G_CALLBACK(on_installer_completed),
                         context) <= 0) {
 ...
 */
void rauc_install(const gchar* bundle,
                  GSourceFunc on_install_notify,
