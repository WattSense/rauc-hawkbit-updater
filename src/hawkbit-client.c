 ...
 */
static const char* HTTPMethod_STRING[] = {"GET", "HEAD", "PUT", "POST", "PATCH", "DELETE"};
 ...
 */
gint get_binary(const gchar* download_url,
                const gchar* file,
                gint64 filesize,
    ...
    if (hawkbit_config->auth_token) {
        gchar* auth_token =
            g_strdup_printf("Authorization: TargetToken %s", hawkbit_config->auth_token);
 ...
 */
gint rest_request(enum HTTPMethod method,
                  const gchar* url,
 ...
 */
void json_build_status(JsonBuilder* builder,
                       const gchar* id,
                       const gchar* detail,
                       const gchar* result,
 ...
 */
gboolean feedback_progress(const gchar* url,
                           const gchar* id,
                           gint progress,
                           const gchar* detail,
                           GError** error) {
...
gchar* build_api_url(gchar* path) {
    return g_strdup_printf("%s://%s%s", hawkbit_config->ssl ? "https" : "http",
                           hawkbit_config->hawkbit_server, path);
}

gboolean hawkbit_progress(const gchar* msg) {
    gchar* feedback_url = NULL;
    gchar* path = NULL;
    if (action_id) {
        path = g_strdup_printf("/%s/controller/v1/%s/deploymentBase/%s/feedback",
...
gboolean install_complete_cb(gpointer ptr) {
    ...
    if (action_id) {
        ...
        if (result->install_success) {
            ...
            feedback(feedback_url, action_id, "Software bundle installed successful.", "success",
                     "closed", NULL);
        } else {
            g_critical("Failed to install software bundle.");
            feedback(feedback_url, action_id, "Failed to install software bundle.", "failure",
                     "closed", NULL);
        }
        g_free(feedback_url);
...
gpointer download_thread(gpointer data) {
    ...
    if (status != 200) {
        gchar* msg_err = g_strdup_printf("Download failed. Status: %d", status);
        g_critical("%s", msg_err);
        feedback(artifact->feedback_url, action_id, msg_err, "failure", "closed", NULL);
        g_free(msg_err);
        goto down_error;
    }
    ...
    if (g_strcmp0(artifact->sha1, checksum.checksum_result)) {
        gchar* msg_ck =
            g_strdup_printf("Software: %s V%s. Invalid checksum: %s expected %s", artifact->name,
                            artifact->version, checksum.checksum_result, artifact->sha1);
        feedback(artifact->feedback_url, action_id, msg_ck, "failure", "closed", NULL);
        g_critical("%s", msg_ck);
        g_set_error(error, 1, 25, "%s", msg_ck);
        g_free(msg_ck);
        status = -3;
        goto down_error;
    }
...
gboolean process_deployment(JsonNode* req_root, GError** error) {
    ...
    gchar* resource_id = g_strdup(groups[2]);
    g_strfreev(groups);
    g_free(deployment);
    ...
    if (json_chunks == NULL || json_array_get_length(json_chunks) == 0) {
        feedback(feedback_url, action_id, "Failed to parse deployment resource.", "failure",
                 "closed", NULL);
    ...
    g_message("New software ready for download. (Name: %s, Version: %s, Size: %" G_GINT64_FORMAT
              ", URL: %s)",
...
void hawkbit_start_service_sync() {
    ...
    timeout_source = g_timeout_source_new(10000);  // pull every 10 seconds
